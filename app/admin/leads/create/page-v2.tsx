...
async function parsePDFFile(_file: File): Promise<BulkLeadRow[]> {
  // PDF parsing requires server-side processing; we'll send raw file to a dedicated endpoint.
  // For now, return a helpful error guiding the user.
  throw new Error(
    'PDF bulk import requires server-side parsing. Please use Excel (.xlsx) or CSV instead, or use the Google Sheets import.',
  );
}

async function parseDOCXFile(_file: File): Promise<BulkLeadRow[]> {
  throw new Error(
    'Word (.docx) bulk import requires server-side parsing. Please use Excel (.xlsx) or CSV instead, or use the Google Sheets import.',
  );
}
...
function mapRowsToLeads(rows: any[]): BulkLeadRow[] {
  return rows.map((row, idx) => {
    ...
    const priority: Priority = ['HIGH', 'MEDIUM', 'LOW'].includes(rawPriority)
