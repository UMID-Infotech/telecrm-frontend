// teleCRM/app/leads/create/page.tsx
'use client';
...
export default function CreateLeadPage() {
  ...
  const addField = () => {
    setFields([...fields, { id: crypto.randomUUID(), key: '', type: 'TEXT', value: '' }]);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      showToast(err?.response?.data?.message ?? 'Failed to create lead', 'destructive');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          {fields.length === 0 && (
            <p className="text-sm text-muted-foreground text-center py-4">No extra fields</p>
          )}
          {fields.map((field) => (
            <div key={field.id} className="border rounded-lg p-4 space-y-3">
              <div className="flex gap-2">
                ...
                <div className="w-36 space-y-1">
                  ...
                  <Select
                    value={field.type}
                    onValueChange={(v) => updateField(field.id, { type: v as FieldType, value: '' })}
                  >
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>
                ...
                <Button variant="ghost" size="icon" onClick={() => removeField(field.id)} className="self-end text-destructive">
                  <Trash2 size={16} />
                </Button>
              </div>
              <div className="space-y-1">
                ...
                {field.type === 'NUMBER' && (
                  <Input type="number" value={field.value} onChange={(e) => updateField(field.id, { value: Number(e.target.value) })} />
                )}
                {field.type === 'BOOLEAN' && (
                  <Select value={String(field.value)} onValueChange={(v) => updateField(field.id, { value: v === 'true' })}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>
                ...
                {field.type === 'DATE' && (
                  <Input type="date" value={field.value} onChange={(e) => updateField(field.id, { value: e.target.value })} />
                )}
                {field.type === 'DROPDOWN' && (
                  <Input
