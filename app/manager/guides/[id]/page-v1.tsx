// telecrm-frontend/app/guides/[id]/page.tsx
'use client';
...
import { ArrowLeft, BookOpen, CalendarDays, Clock, Tag, User } from 'lucide-react';
...
export default function GuideViewPage() {
  ...
  useEffect(() => {
    async function fetchBlog() {
      try {
        const res = await fetch(`${API_BASE}/guide-blogs/${id}`, {
      ...
      <article className="max-w-3xl mx-auto px-4 pb-20">
        ...
        <div className="flex flex-wrap items-center gap-2 mb-4">
          {blog.visibleTo.map((v) => (
            <Badge key={v} className="bg-blue-50 text-blue-700 border-blue-200 text-xs">
              {v}
            </Badge>
          ))}
