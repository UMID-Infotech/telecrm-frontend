  ...
  const handleSubmit = async () => {
    ...
    try {
      if (status === 'CONVERTED') {
        ...
        // Backend requires type + callDisposition for CALL_LOG,
        // but STATUS_CHANGE is a separate type with no callDisposition needed.
        ...
        await api.post('/agent/conversation', {
          ...
          notes: `Lead marked as ${status === 'LOST' ? 'Lost' : 'Not Interested'}. Reason: ${lossReason}.${closureNotes.trim() ? ` Notes: ${closureNotes.trim()}` : ''}`,
        });
      }
      ...
      showToast(err?.response?.data?.message ?? 'Failed to update lead status', 'destructive');
    } finally {
      setLoading(false);
    }
  };
              ...
              <>
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="closureNotes" className="font-semibold text-slate-700">
                    Closure Notes <span className="text-muted-foreground font-normal text-xs">(optional)</span>
                  </Label>
                  <Textarea
                    id="closureNotes"
